...
inline void __attribute__((always_inline)) handle_line(const uint8_t* data, HashBin* hmap, size_t &data_idx)
{
  ...
  if (unlikely(data[pos] != ';')) {
    __m128i chars_2 = _mm_loadu_si128((__m128i*)(data + 17));
    __m128i compared_2 = _mm_cmpeq_epi8(chars_2, separators);
    uint32_t separator_mask2 = _mm_movemask_epi8(compared_2);

    if (likely(separator_mask2)) pos = 17 + __builtin_ctz(separator_mask2);
    else {
        pos = 32;
        while (data[pos] != ';') pos++;
    }
    ...
    while (hmap[myhash].len > 0) {
      ...
      if (likely(hmap[myhash].len == len)) {
        ...
        while (idx + 32 < len) {
          ...
          __m256i bin_chars = _mm256_loadu_si256((__m256i*)(hmap[myhash].key + idx));
        ...
        if (likely(idx <= 64)) {
          ...
          // len must be >= 97
          bool equal = true;
          for (int i = idx; i < len; i++) if (data[i] != hmap[myhash].key[i]) {
              equal = false;
              break;
          }
