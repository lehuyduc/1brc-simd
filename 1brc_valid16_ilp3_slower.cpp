...
void handle_line_raw(int tid, const uint8_t* data, size_t from_byte, size_t to_byte, size_t file_size)
{
  ...
  size_t idx2 = start_idx + BYTES_PER_THREAD * 2;
  
  find_next_line_start(data, to_byte, idx1);
  find_next_line_start(data, to_byte, idx2);

  size_t end_idx0 = idx1 - 1;
  size_t end_idx1 = idx2 - 1;
  size_t end_idx2 = to_byte;  

  while (likely(idx0 < end_idx0 && idx1 < end_idx1 && idx2 < end_idx2)) {
    handle_line<false>(data + idx0, hmaps[tid], idx0);
    handle_line<false>(data + idx1, hmaps[tid], idx1);
    handle_line<false>(data + idx2, hmaps[tid], idx2);
  }
...
// Using 32 threads
// Malloc cost = 0.006703
// init mmap file cost = 0.012453ms
// Parallel process file cost = 437.07ms
// Aggregate stats cost = 1.7223ms
// Output stats cost = 0.729723ms
// Runtime inside main = 439.583ms
// Time to munmap = 151.195
// Time to free memory = 4.19996
...
// Using 32 threads
// Malloc cost = 0.006573
// init mmap file cost = 0.012504ms
// Parallel process file cost = 443.717ms
// Aggregate stats cost = 1.91312ms
// Output stats cost = 1.26585ms
// Runtime inside main = 446.964ms
// Time to munmap = 156.736
// Time to free memory = 4.20748
...
// init mmap file cost = 0.018765ms
// Parallel process file cost = 442.515ms
// Aggregate stats cost = 1.69437ms
// Output stats cost = 0.970893ms
// Runtime inside main = 445.26ms
// Time to munmap = 152.41
// Time to free memory = 4.15156
