...
size_t N = 3'200'000'000ULL;
constexpr size_t N_LOOP = 5;
int* A;
const int* B;

void __attribute__ ((noinline)) func(int* __restrict__ A, const int* __restrict__ B, size_t from_int, size_t to_int, int t)
{
  dummies[t] = t;
  for (size_t i = from_int; i < to_int; i++) A[i] = B[i];
  A[dummies[t]] = dummies[t];  
}

void parallel_copy(size_t tid, int* __restrict__ A, const int* __restrict__ B)
{
    size_t from_int = 100'000'000 * tid;
    size_t to_int = 100'000'000 * (tid + 1);

    for (size_t t = 1; t <= N_LOOP; t++) {   
      func(A, B, from_int, to_int, t);
      dummies[t] = A[rand() % N];
    }
}
